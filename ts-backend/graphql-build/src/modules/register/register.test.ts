import { duplicatedEmail, emailNotLongEnough, notValidEmail, passwordIsNotLongEnough } from './errorMsgs';
...
describe('Register user', () => {
  it('Check for duplicated emails', async () => {
    ...
    const response2: any = await request(process.env.TEST_HOST as string, registerMutation(email, password));
  ...
  it('Catch bad email', async () => {
    const response3: any = await request(process.env.TEST_HOST as string, registerMutation('b', password));
  ...
  it('Catch bad password', async () => {
    const response4: any = await request(process.env.TEST_HOST as string, registerMutation(email, 'x'));
  ...
  it('Catch bad password and bad email', async () => {
    const response5: any = await request(process.env.TEST_HOST as string, registerMutation('b', 'x'));
